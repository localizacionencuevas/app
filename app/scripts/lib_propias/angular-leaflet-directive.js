(function () {
  ...
  angular.module('leaflet-directive', []).directive('leaflet', [
    ...
    'leafletHelpers',
    'leafletEvents',
    function ($window,$q, leafletData, leafletMapDefaults, leafletHelpers, leafletEvents) {
      var _leafletMap;
      return {
        restrict: 'EA',
        replace: true,
        ...
        link: function (scope, element, attrs) {
          ...
          $('#mapa').css('height', angular.element($window).height() - 50 + 'px');
        ...
        link: function (scope, element, attrs, controller) {
          ...
          controller.getMap().then(function (map) {
            ...
            for (layerName in layers.overlays) {
              ...
              // console.log(leafletLayers.overlays[layerName]);
  ...
  angular.module('leaflet-directive').directive('controls', [
    ...
    'leafletHelpers',
    'leafletData',
    function ($log, leafletHelpers, leafletData) {
      return {
        restrict: 'A',
        scope: false,
        replace: false,
        ...
        link: function (scope, element, attrs, controller) {
          ...
          var isDefined = leafletHelpers.isDefined, leafletLayers = {}, leafletScope = controller.getLeafletScope(), layers = leafletScope.layers, controls = leafletScope.controls;
          controller.getMap().then(function (map) {
            if (isDefined(L.Control.Draw) && isDefined(controls.draw)) {
              var drawCtrlInst, drawCtrlDisp;
              leafletData.getLayers().then(function (layers){
                leafletLayers = layers;
                console.log(leafletLayers);

                // var options1 = { edit: { featureGroup: leafletLayers.overlays['capaInstalaciones'], remove: false } };
                // angular.extend(options1, controls.draw.drawCtrlInst.options);
                // drawCtrlInst = new L.Control.Draw(options1);

                // var options2 = { edit: { featureGroup: leafletLayers.overlays['capaDispositivos'], remove: true } };
                // angular.extend(options2, controls.draw.drawCtrlDisp.options);
                // drawCtrlDisp = new L.Control.Draw(options2);

                // map.addControl(drawCtrlDisp);
              });
              ...
              // map.addControl(drawCtrlDisp);
              // if (map.hasLayer('Google Streets')) {
              //   console.log(layer);
              // }
              // var drawCtrlInst = new L.Control.Draw(controls.draw.drawCtrlInst.options);
              // var drawCtrlDisp = new L.Control.Draw(controls.draw.drawCtrlDisp.options);
            }

            // leafletScope.$watch('controls.draw.drawCtrlInst.options.visible', function (newValue, oldValue) {
            //   if (!newValue) {
                
            //     if (newValue !== oldValue) {
            //       drawCtrlInst.removeFrom(map);
            //     }
            //   }
            //   else if(newValue){
            //     drawCtrlInst.addTo(map);
            //     console.log(drawCtrlInst);
            //   }
            // });

            // leafletScope.$watch('controls.draw.drawCtrlDisp.options.visible', function (newValue, oldValue) {
            //     var options1 = { edit: { featureGroup: leafletLayers.overlays['capaDispositivos'], remove: false } };
            //     var options2 = { edit: { featureGroup: leafletLayers.overlays['capaDispositivos'], remove: true } };
            //     var permiso = leafletScope.controls.draw.drawCtrlDisp.options.puedeBorrar;
            //   if (!newValue) {

            //     if (newValue !== oldValue) {
            //       drawCtrlDisp.removeFrom(map);
            //     }
                
            //   }else if(newValue){
            //     if (!permiso) {
            //       angular.extend(options1, controls.draw.drawCtrlDisp.options);
            //       drawCtrlDisp = new L.Control.Draw(options1);
            //       // drawCtrlDisp.options.edit.remove = false;
            //     }else{
            //       angular.extend(options2, controls.draw.drawCtrlDisp.options);
            //       drawCtrlDisp = new L.Control.Draw(options2);
            //     }

            //     drawCtrlDisp.addTo(map);
            //     console.log(drawCtrlDisp);
            //   }
            // });

            if (isDefined(L.Control.Scale) && isDefined(controls.scale)) {
              var options = {maxWidth:120};
              angular.extend(options, controls.scale.options);
              var scaleControl = new L.Control.Scale(options);
            }

            leafletScope.$watch('controls.scale.options.visible', function (visible, oldValue) {
              if (!visible) {
                if (visible !== oldValue) {
        ...
        setDefaults: function (userDefaults, scopeId) {
          ...
          if (isDefined(userDefaults)) {
            ...
            if (isDefined(userDefaults.center)) {
              newDefaults.center = userDefaults.center;
            }
  ...
  angular.module('leaflet-directive').factory('leafletPathsHelpers', [
    ...
    function ($rootScope, $log, leafletHelpers) {
      ...
      return {
        ...
        convertToLeafletLatLng: _convertToLeafletLatLng
  ...
  angular.module('leaflet-directive').factory('leafletMarkersHelpers', [
    ...
    function ($rootScope, leafletHelpers, $log) {
      ...
      var createLeafletIcon = function (iconData) {
        ...
        if (!isDefined(iconData.shadowUrl)) {
          iconData.shadowUrl = base64shadow;
        }
